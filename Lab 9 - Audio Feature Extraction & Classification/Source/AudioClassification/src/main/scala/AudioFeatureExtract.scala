...
object AudioFeature extends Enumeration {
  ...
  object AudioFeatureExtract {
   ...
   // val meanZCR = calculateMean(f);
    ...
    println(f.length +" " + f1.length +" " + f2.length +" "+ f3.length +" "+ f4.length +" "+ f5.length +" "+ f6.length)

   // val str = meanZCR + ";" + meanMFCC + ";" + meanSpectralRollOff + ";" + meanPeakValue + ";" + meanRMS + ";" + meanCompactness + ";"
   // println("Features extracted are : " + str)
    val str = "hello"
    val pos = new File(path).getName.lastIndexOf(".")
    val name = new File(path).getName.substring(0,pos)
      println(name)
      val file = new File("TrainingData.txt")
    val bw = new BufferedWriter(new FileWriter(file,true))

      for(i<-0 until f.length)
    {
      bw.append(name+":"+f(i)+" "+f1(i)+" "+f2(i)+" "+f3(i)+" "+f4(i)+" "+f5(i)+" "+f6(i))
      bw.newLine()
    }
      bw.close()
    str
  ...
  def feature(audio: AudioSamples, i: AudioFeature.Value): Array[Double] = {
    ...
    val featureMeanSampleArray  = new Array[Double](samples.length)
  ...
  def main(args: Array[String]): Unit = {
    val features1 = AudioFeatureExtraction("data/Lion.wav")
